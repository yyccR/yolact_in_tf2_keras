...
class HeadLayer(tf.keras.layers.Layer):
    def __init__(self):
        ...
        self.conv_cls = tf.keras.layers.Conv2D(len(self.aspect_ratios)*self.num_classes,kernel_size=3, padding='same')(p3_x)
        self.conv_mask = tf.keras.layers.Conv2D(len(self.aspect_ratios)*self.mask_proto_channels,kernel_size=3, padding='same')(p3_x)
